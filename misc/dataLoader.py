...
class train(data.Dataset) :  # torch wrapper
    ...
    def __getitem__(self, index) :
        ...
        ans = np.zeros((self.total_qa_pairs, self.ans_length + 1))
        ans_target = np.zeros((self.total_qa_pairs, self.ans_length + 1))
        ques_ori = np.zeros((self.total_qa_pairs, self.ques_length))

        opt_ans = np.zeros((self.total_qa_pairs, self.negative_sample, self.ans_length + 1))
